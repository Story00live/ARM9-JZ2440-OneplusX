...
/**********************************************************************************************************
 @Function			static void S3C2440_NorFlash_Wait_Ready(unsigned int addr)
 @Description			S3C2440_NorFlash_Wait_Ready					: Nor Flash 等待空闲
 @Input				addr
 @Return				void
**********************************************************************************************************/
static void S3C2440_NorFlash_Wait_Ready(unsigned int addr)
{
	unsigned int val;
	unsigned int pre;
...
static void S3C2440_NorFlash_doEraseNorFlash(void)
{
	...
	S3C2440_NorFlash_Write_Command(0x0555, 0xAA);
	S3C2440_NorFlash_Write_Command(0x02AA, 0x55);
	S3C2440_NorFlash_Write_Command(eraseAddress >> 1, 0x30);					// 发出扇区地址
	
	/* -等待擦除完成- */
	S3C2440_NorFlash_Wait_Ready(eraseAddress >> 1);
}
...
static void S3C2440_NorFlash_doWriteNorFlash(void)
{
	...
	while (writeStr[lowAddr] && writeStr[highAddr]) {
		writeVal = writeStr[lowAddr] + (writeStr[highAddr] << 8);
