...
static int TouchScreenX = 0;
static int TouchScreenY = 0;
static int TouchScreenPressures = 0;
static int TouchScreenDataValid = DATAVALID;
...
/**********************************************************************************************************
 @Function			void TouchScreen_Report_TouchScreen_XY(int x, int y, int pressure)
 @Description			TouchScreen_Report_TouchScreen_XY
 @Input				x
					y
					pressure
 @Return				void
...
{
	if (TouchScreenDataValid != DATAHAVED) {
		TouchScreenX = x;
		TouchScreenY = y;
		TouchScreenPressures = pressure;
		TouchScreenDataValid = DATAHAVED;
	}
}

/**********************************************************************************************************
 @Function			void TouchScreen_ReadRaw_TouchScreen_XY(int* px, int* py, int* ppressure)
 @Description			TouchScreen_ReadRaw_TouchScreen_XY
 @Input				*x
					*y
					*pressure
 @Return				void
...
{
	while (TouchScreenDataValid == DATAVALID);
	*px = TouchScreenX;
	*py = TouchScreenY;
	*ppressure = TouchScreenPressures;
	TouchScreenDataValid = DATAVALID;
}
...
/**********************************************************************************************************
 @Function			void TouchScreen_Enter_Wait_Pen_Up_Mode(void)
 @Description			TouchScreen_Enter_Wait_Pen_Up_Mode
 @Input				void
 @Return				void
**********************************************************************************************************/
...
/**********************************************************************************************************
 @Function			int  TouchScreen_Check_Auto_Measure_Mode(void)
 @Description			TouchScreen_Check_Auto_Measure_Mode
 @Input				void
 @Return				int
**********************************************************************************************************/
static int  TouchScreen_Check_Auto_Measure_Mode(void)
{
...
void S3C2440_TouchScreen_Timer_Irq(void)
{
	...
	/* -如果为"自动测量模式", 不必启动ADC- */
	if (TouchScreen_Check_Auto_Measure_Mode()) return;
	
	/* -只有在"等待中断模式"下才可使用ADCDATA0'BIT15来判断触摸屏状态- */
	if (ADCDAT0 & (1<<15)) {
		/* -松开- */
		Debug_Printf("[TouchScreen Timer: Pen up.]");
		...
		/* -按下- */
		Debug_Printf("[TouchScreen Timer: Pen down.]");
